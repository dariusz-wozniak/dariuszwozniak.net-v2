...
summary: In some cases, there are classes with constructors that are impossible to mock with proxy-based mocking frameworks (such as Moq, FakeItEasy, NSubstitute, etc). In this article, I will show you how to create a class that is impossible to mock.
---

In some cases, there are classes with constructors that are impossible to mock with proxy-based mocking frameworks (such as Moq, FakeItEasy, NSubstitute, etc). In this article, I will show you how to create a class that is impossible to mock.
...
In the above case, we cannot easily mock the class `ClassWithDifficultCtorToMock` because it has a private constructor. What we can do is to create a class using the `FormatterServices.GetUninitializedObject`:
