...
Things will be more complicated when the property has an actual references to other entities. In this case, you should also remove the references from the database. You can do that manually or by using custom code (e.g. with `IContentRepository.GetReferencesToContent` or `IContentModelUsage`).
