...
date: '2023-12-20'
draft: false
thumbnail: '/static/images/blog/optimizely/shopping-cart.png'
tags: ['EPiServer', 'Optimizely']
summary: By default, Optimizely invokes its internal validators for the cart processing. This article describes how to skip this validation in scenarios where custom validation logic is in place or we don't want UI to update the cart when viewing it.
...
So, when the backoffice user wants to view carts and validation fails then the cart might be mutated and the user will see the altered cart. It's an UX design flaw that violates CQRS and should not happen as viewing the cart should not mutate it. There should be a separate action button for validation (and thus, potential mutation) the cart. The example scenario is that variant does not have _max quantity_ setting and user added it to the cart. Entering the Commerce UI will remove the line item from the cart because of the validation. The message goes as the following:

<ArticleImage src={"/static/images/blog/optimizely/shopping-cart.png"} alt={"Cart view"} />

> The product XYZ does not have a valid max quantity setting and was removed from your cart.

This article describes how to skip this default validation in scenarios where custom validation logic is in place.

## Under the hood

The class responsible for the cart validation is the `EPiServer.Commerce.Order.OrderValidationService`.

It has the following validations methods:
...
* `UpdateInventoryOrRemoveLineItems`:
    * runs again after applying discounts (some line items might be added after applying promotions)

These are most of the validations listed. And, as we can see, the above methods might alter the cart and its contents, i.e. remove line items from the cart.
...
In order to skip the validation, you may just substitute dependency by registering own implementation of `IOrderValidationService` in the IoC container [@Mai2023].

```csharp
services.AddTransient<OrderValidationService, CustomOrderValidationService>();
```

The next step would be to override the `ValidateOrder` method and use the own logic. For some cases, you might need to skip the validation only for a specific cart. In this case, you might use `orderGroup` parameter of the above method and its cart name (e.g. "Wishlist") to determine if the validation should be skipped:
...
If you want to skip the validation only for the cart processing in the Commerce UI then you might use the `HttpContext` to determine if the request is coming from the backoffice:
