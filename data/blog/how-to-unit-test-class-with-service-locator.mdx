...
date: '2023-11-09'
draft: true
thumbnail: '/static/images/blog/unit-test-service-locator.png'
tags: ['EPiServer', 'Optimizely', 'unit test', 'dependency injection']
summary: In this post I will present how to write unit test for a class with dependency to ServiceLocator, with the real world example of Optimizely.
---
...
In this post I will present how to write unit test for a class with dependency to `ServiceLocator`, with the real world example of Optimizely. Note Optimizely uses Microsoft.Extensions.DependencyInjection, but the same approach can be used for other DI containers. Also, the scenario apply for non-Optimizely projects.
...
The above class is difficult to unit test, because:
...
* `ServiceLocator` is a static class, so it is not possible to mock it with free proxy-based test frameworks like `Moq` or `NSubstitute`.
