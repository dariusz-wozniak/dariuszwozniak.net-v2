...
Is `catch` the same as `catch(Exception)`? Not exactly. There is a small subtle nuance that causes `catch(Exception)` to be not hit. Chances that it happen are, however, close to zero very close to zero.
...
- `catch` is the same `catch(Exception)` unless you're using C# version lower than 2.0 and non-CLS-compliant exceptions. Or, you're using the `WrapNonExceptionThrows` flag set to true in the `RuntimeCompatibility` attribute.
